...
passport.use(new GoogleStrategy({
    ...
    callbackURL: "http://localhost:4000/gok",
        passReqToCallback   : true
    },
    function(request, accessToken, refreshToken, profile, done) {
        var tempNum = utils.uid(10);
        UserService.authGoogleUser('g-password',profile.email,tempNum,profile.name.givenName,profile.name.familyName,tempNum,function(err,token){
            if(err){
                return done(err);
            }
            else{
                profile.hd_map_token=token.value;
